...
from .mongo_serializers import PublisherSerializer
...
def publishers_list_create(request):
...
@permission_classes([AllowAny])
def publishers_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = PublisherSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
